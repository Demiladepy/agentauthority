...
function agentLog(agentId, level, msg) {
    const names = {
...
async function main() {
    ...
    const makeAgent = (id, name, role, trust) => {
    ...
    const rootAuth = await manager.createRootAuthority('orchestrator', 'orchestrator', rootPolicy, 'solana:mainnet');
    ...
    if (computePayment.paid)
        success(`${fmt(USDC(2.00))} — backtesting compute`);
    await sleep(300);
    // --------------------------------------------------------
...
//# sourceMappingURL=research-workflow.js.map