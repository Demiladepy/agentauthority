...
class PolicyEngine {
    ...
    validate(intent) {
        ...
        if (!authority) {
            return { valid: false, reason: 'Authority not found', code: types_1.ValidationErrorCode.INVALID_SIGNATURE };
        }
        // 1. Check status
        if (authority.status === 'revoked') {
            this.emitRejectEvent(authority, intent, 'Authority has been revoked');
            return { valid: false, reason: 'Authority has been revoked', code: types_1.ValidationErrorCode.AUTHORITY_REVOKED };
        }
        if (authority.status === 'exhausted') {
            this.emitRejectEvent(authority, intent, 'Authority budget exhausted');
            return { valid: false, reason: 'Authority budget exhausted', code: types_1.ValidationErrorCode.AUTHORITY_EXHAUSTED };
        }
        // 2. Check expiry
        if (Date.now() > authority.policy.expiresAt) {
            ...
            return { valid: false, reason: 'Authority has expired', code: types_1.ValidationErrorCode.AUTHORITY_EXPIRED };
        }
        ...
        if (authority.policy.allowedPrograms.length > 0 &&
            ...
            return {
                ...
                code: types_1.ValidationErrorCode.PROGRAM_NOT_ALLOWED,
            };
        }
        ...
        if (intent.destination &&
            ...
            return {
                ...
                code: types_1.ValidationErrorCode.DESTINATION_NOT_ALLOWED,
            };
        }
        ...
        if (intent.amount > authority.policy.maxTransactionSize) {
            ...
            return {
                ...
                code: types_1.ValidationErrorCode.EXCEEDS_TRANSACTION_LIMIT,
            };
        }
        ...
        if (authority.spent + intent.amount > effectiveBudget) {
            ...
            return {
                ...
                code: types_1.ValidationErrorCode.EXCEEDS_SPENDING_LIMIT,
            };
        }
        // 7. Check rate limit
        const now = Date.now();
        ...
        if (recentTxCount >= authority.policy.rateLimit.maxTransactions) {
            ...
            return {
                ...
                code: types_1.ValidationErrorCode.RATE_LIMIT_EXCEEDED,
            };
        }
    ...
    // Delegation
    // -------------------------------------------------------
    validateDelegation(parentAuthorityId, requestedAmount, requestedPolicy) {
        const parent = this.authorities.get(parentAuthorityId);
        if (!parent) {
            return { valid: false, reason: 'Parent authority not found', code: types_1.ValidationErrorCode.INVALID_SIGNATURE };
        }
        if (parent.status !== 'active') {
            return { valid: false, reason: `Parent authority is ${parent.status}`, code: types_1.ValidationErrorCode.AUTHORITY_REVOKED };
        }
        ...
        if (parent.depth >= parent.policy.maxDelegationDepth) {
            return {
                ...
                code: types_1.ValidationErrorCode.DELEGATION_DEPTH_EXCEEDED,
            };
        }
        ...
        if (requestedAmount > availableForDelegation) {
            return {
                ...
                code: types_1.ValidationErrorCode.INSUFFICIENT_DELEGATION_BUDGET,
            };
        }
        ...
        if (requestedPolicy.expiresAt && requestedPolicy.expiresAt > parent.policy.expiresAt) {
            return {
                ...
                code: types_1.ValidationErrorCode.AUTHORITY_EXPIRED,
            };
        }
    ...
    // Revocation
    // -------------------------------------------------------
    revoke(authorityId, reason = 'Manual revocation') {
    ...
    emitRejectEvent(authority, intent, reason) {
        this.emitEvent({
            id: (0, uuid_1.v4)(),
            timestamp: Date.now(),
    ...
    // Helpers
    // -------------------------------------------------------
    getDelegationPath(authority) {
        const path = [authority.id];
