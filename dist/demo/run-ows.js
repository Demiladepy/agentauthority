...
const signer_1 = require("../ows/signer");
// ============================================================
// CONSTANTS
// ============================================================
...
function success(text) { console.log(`  ${COLORS.green}✓${COLORS.reset} ${text}`); }
function fail(text) { console.log(`  ${COLORS.red}✗${COLORS.reset} ${text}`); }
function info(text) { console.log(`  ${COLORS.dim}→${COLORS.reset} ${text}`); }
function warn(text) { console.log(`  ${COLORS.yellow}⚠${COLORS.reset} ${text}`); }
...
async function main() {
    ...
    const [orchSigner, researchSigner, traderSigner, scraperSigner] = await Promise.all([
        (0, signer_1.createOWSWallet)(walletNames.orchestrator, 'solana:mainnet'),
        (0, signer_1.createOWSWallet)(walletNames.researcher, 'solana:mainnet'),
        (0, signer_1.createOWSWallet)(walletNames.trader, 'solana:mainnet'),
        (0, signer_1.createOWSWallet)(walletNames.scraper, 'solana:mainnet'),
    ]);
    ...
    const makeIdentity = (id, name, role, trust, pubkey) => ({
    ...
    if (pay1.paid)
        success(`Payment approved: ${formatUSDC(USDC(0.50))} for wallet risk profile`);
    ...
    if (pay2.paid)
        success(`Payment approved: ${formatUSDC(USDC(1.00))} for vesting audit`);
    await sleep(300);
    // --------------------------------------------------------
    ...
    if (scraperPay.paid)
        success(`DataScraper payment at depth 2: ${formatUSDC(scraperPayment.amount)}`);
    await sleep(300);
    // --------------------------------------------------------
    ...
    if (t1.success)
        success(`Trade approved: ${formatUSDC(USDC(8))} — remaining: ${formatUSDC(t1.remainingBudget)}`);
    ...
    if (scraperStatus?.status === 'revoked')
        success('DataScraper cascaded — also revoked');
    const postRevoke = await scraper.payX402(scraperPayment);
    if (!postRevoke.paid)
        fail(`BLOCKED post-revocation: ${postRevoke.error}`);
    // --------------------------------------------------------
    // FINAL REPORT
    // --------------------------------------------------------
    banner('FINAL STATUS REPORT');
    for (const [name, agent] of [['Orchestrator', orchestrator], ['Researcher', researcher], ['Trader', trader], ['DataScraper', scraper]]) {
    ...
    const counts = auditTrail.reduce((acc, e) => { acc[e.eventType] = (acc[e.eventType] || 0) + 1; return acc; }, {});
...
//# sourceMappingURL=run-ows.js.map