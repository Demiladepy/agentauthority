...
function botLog(id: string, level: string, msg: string): void {
...
async function main() {
  ...
  const makeBot = (id: string, name: string, role: string, trust: number) => {
  ...
  const fundPolicy: SpendingPolicy = {
    maxSpend: USDC(1000),
    tokenMint: USDC_MINT,
  ...
  if (arb1.success) success(`Arb trade 1: ${fmt(USDC(80))} via Jupiter`);
  ...
  if (arb2.success) success(`Arb trade 2: ${fmt(USDC(80))} via Raydium`);
  ...
  if (stake1.success) success(`Staked: ${fmt(USDC(75))} USDC → mSOL via Marinade`);
