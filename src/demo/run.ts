...
import { AutonomousAgent, AgentTask, TaskResult } from '../agents/autonomous-agent';
...
async function main() {
  ...
  for (const eventType of auditEvents) {
    engine.on(eventType as any, (event: AuditEvent) => {
  ...
  const funding = await fundAgentWallet(
    orchestrator.signer.getPublicKey(),
    100,
  ...
  const dataPayment1: X402PaymentRequired = {
  ...
  const dataPayment2: X402PaymentRequired = {
  ...
  const negotiation = await newAgent.requestPermission(
    rootAuthority.id,
    'orchestrator',
    {
