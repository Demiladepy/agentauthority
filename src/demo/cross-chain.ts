...
import { SpendingPolicy, AgentIdentity } from '../core/types';
import { CHAIN_INFO } from '../chains/chain-adapter';
import { BASE_USDC, BASE_UNISWAP_V3, BASE_AAVE_V3 } from '../chains/evm-authority';
...
function chainBadge(chain: string): string {
  if (chain.startsWith('solana')) return `${C.magenta}[Solana]${C.reset}`;
  if (chain === 'eip155:8453') return `${C.cyan}[Base]${C.reset}`;
  return `[${chain}]`;
}
...
function fmtUSDC(lamports: bigint): string {
...
async function main() {
  ...
  const orchestratorSigner = new Ed25519Signer();
  ...
  const solEngine  = new PolicyEngine();
  const solManager = new AuthorityManager(solEngine);
  const solX402    = new X402Handler(solEngine);

  solManager.registerAgent(
    { id: 'orchestrator', name: 'Orchestrator', pubkey: orchPubkey,
      role: 'Root authority holder', trustScore: 100, totalSpends: 0, totalRejections: 0 },
    orchestratorSigner
  );

  const researcherSolSigner = new Ed25519Signer();
  solManager.registerAgent(
    { id: 'researcher', name: 'Researcher', pubkey: researcherSolSigner.getPublicKey(),
      role: 'Data researcher', trustScore: 80, totalSpends: 0, totalRejections: 0 },
    researcherSolSigner
  );

  const solPolicy: SpendingPolicy = {
    maxSpend: USDC(50),
  ...
  const solRootAuth = await solManager.createRootAuthority(
    'orchestrator', 'orchestrator', solPolicy, 'solana:mainnet'
  );
  ...
  const baseManager = new AuthorityManager(baseEngine);
  const baseX402    = new X402Handler(baseEngine);

  // Same orchestrator (same signer), but registered in the BASE engine
  baseManager.registerAgent(
    { id: 'orchestrator', name: 'Orchestrator', pubkey: orchPubkey,
      role: 'Root authority holder', trustScore: 100, totalSpends: 0, totalRejections: 0 },
    orchestratorSigner
  );

  const traderBaseSigner = new Ed25519Signer();
  baseManager.registerAgent(
    { id: 'trader', name: 'Trader', pubkey: traderBaseSigner.getPublicKey(),
  ...
  const basePolicy: SpendingPolicy = {
    maxSpend: USDC(50),
    tokenMint: BASE_USDC,
    allowedPrograms: [BASE_UNISWAP_V3, BASE_AAVE_V3, X402_BASE],
    allowedDestinations: [],
    expiresAt: Date.now() + 2 * 60 * 60 * 1000,
    maxRedelegation: USDC(30),
    maxDelegationDepth: 2,
  ...
  const baseRootAuth = await baseManager.createRootAuthority(
  ...
  info(`Token: USDC on Base (${BASE_USDC.slice(0, 14)}...)`);
  ...
  const researcherSolAuth = await solManager.delegate(
    solRootAuth.id, 'orchestrator', 'researcher', USDC(20),
    { allowedPrograms: [ALLIUM, X402_SOL], maxTransactionSize: USDC(5) }
  );
  ...
  if (r1.paid) success(`${chainBadge('solana:mainnet')} Allium payment: ${fmtUSDC(USDC(0.50))} — remaining: ${fmtUSDC(r1.remaining!)}`);
  ...
  if (r2.paid) success(`${chainBadge('solana:mainnet')} Allium payment: ${fmtUSDC(USDC(0.25))} — remaining: ${fmtUSDC(r2.remaining!)}`);
  ...
  const traderBaseAuth = await baseManager.delegate(
    baseRootAuth.id, 'orchestrator', 'trader', USDC(20),
    { allowedPrograms: [BASE_UNISWAP_V3, X402_BASE], maxTransactionSize: USDC(15) }
  );
  ...
  const t1 = await trader.spend({
    programId: BASE_UNISWAP_V3,
    amount: USDC(10),
    description: 'Swap 10 USDC → WETH via Uniswap V3 on Base',
    destination: '0x' + 'dead'.padStart(40, '0'),
  });
  if (t1.success) success(`${chainBadge('eip155:8453')} Uniswap swap approved: ${fmtUSDC(USDC(10))} — remaining: ${fmtUSDC(t1.remainingBudget!)}`);

  const t2 = await trader.spend({
    programId: BASE_AAVE_V3,
    amount: USDC(5),
    description: 'Supply 5 USDC to Aave V3 on Base',
    destination: BASE_AAVE_V3,
  });
  if (t2.success) success(`${chainBadge('eip155:8453')} Aave supply approved: ${fmtUSDC(USDC(5))} — remaining: ${fmtUSDC(t2.remainingBudget!)}`);
  ...
  console.log(`    Solana: ${CHAIN_INFO['solana:mainnet'].name} — USDC ${CHAIN_INFO['solana:mainnet'].usdcAddress.slice(0, 20)}...`);
  console.log(`    Base:   ${CHAIN_INFO['eip155:8453'].name}   — USDC ${CHAIN_INFO['eip155:8453'].usdcAddress.slice(0, 20)}...`);
